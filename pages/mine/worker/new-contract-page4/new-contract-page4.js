// pages/mine/worker/new-contract-page3/new-contract-page3.j
...
previewCanvasImg() {
  ...
  //上传
  uploadCanvasImg() {
    //20191108添加this to that 解决that not found报错 start
    var that = this;
    //20191108添加this to that 解决that not found报错 end
    wx.canvasToTempFilePath({
      canvasId: 'handWriting',
      fileType: 'png',
      quality: 1, //图片质量
      success(res) {
        ...
        //上传
        console.log("====20191108");
        console.log(host);
        console.log(res.tempFilePath);
        wx.uploadFile({
          url: host + '/upload_files/invitation', // 仅为示例，非真实的接口地址
          filePath: res.tempFilePath,
          name: 'invitation',
          formData: {
            user: 'test'
          ...
          success(res) {
            console.log("====20191108 success res");
              console.log(res);
            const data = res.data
            var img_arr = JSON.parse(res.data);
            // 20191115 start
            that.setData({
              sign_image: that.data.img_arr.concat(img_arr[0])
            })
            // 20191115 end sign-image的赋值方法参照page1.js，否则此值一开始是对的，但是点下一步                    //storage的值变成handwriting。

            // console.log(that.data.sign_image);
            // wx.setStorage({
            //   key: 'contract_page4_data',
            //   data: {
            //     // img_arr: img_arr[0]
            //     sign_image: that.data.sign_image
            //   }
            // })
          },
          fail(res) { 
            console.log("====20191108 fail res");
          console.log(res);
          }
        })
...
subCanvas(){
  ...
  wx.setStorage({
    ...
    success: function (res) {
      wx.navigateTo({
        url: '../../worker/new-contract-page5/new-contract-page5',
      })
    },
  })
